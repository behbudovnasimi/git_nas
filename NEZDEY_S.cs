...
namespace hesabat
{
    public partial class NEZDEY_S : Form
    {
        public NEZDEY_S()
        {
            InitializeComponent();
        ...
        private void button1_Click(object sender, EventArgs e) //Təsdiq 
        {
            if (DT.Rows.Count > 0)
            {
                ...
                if (n_logs.my_logs(p_var.n_uzer_name, "qahali.subscriber-nəzarətçi kodu", n_dat1) == "E") //Loqlama ilkin
                { return; }// Bazada sehv
                
                using (System.IO.StreamWriter file = new System.IO.StreamWriter(@"Text.txt", true))
                {
                    file.WriteLine("SUBID   NEW_NEZ   OLD_NEZ");
                }
                ...
                for (int i = 0; i <= DT.Rows.Count-1; i++)
                {
                    ...
                    {
                        p_var.n_rez = my_metod_0(DT.Rows[i]["SUBID"].ToString(), DT.Rows[i]["NEW_NEZ"].ToString(), DT.Rows[i]["OLD_NEZ"].ToString()); //Subscriber de NEZDEY_S deyiş.

                        if (p_var.n_rez.Substring(0, 1) != "Y")
                        {
                            using (System.IO.StreamWriter file = new System.IO.StreamWriter(@"Text.txt", true))
                            {
                                file.WriteLine(p_var.n_rez + "   " + DT.Rows[i]["NEW_NEZ"].ToString() + "   " + DT.Rows[i]["OLD_NEZ"].ToString());
                            }
                        }
                    }
        ...
        private void NEZDEY_S_Load(object sender, EventArgs e)
        {
            System.IO.File.Delete(@"Text.txt"); //Silinmə
            textEdit1.Text="";
        ...
        public void Fun_I_D(string s1)
        {
            ...
            using (OleDbConnection oConn1 = new OleDbConnection())      // DT - doldurmaq
            {
                ...
                try
                {
                    ...
                    n_str1 = DT.Rows[1]["NEW_NEZ"].ToString();
                    n_str2 = DT.Rows[2]["OLD_NEZ"].ToString();
                }
                catch (Exception ex)
                {
                    DT.Columns.Clear();
                    ...
                    {
                        button1.Enabled = false;
                        MessageBox.Show(@"Seçilmiş xlsx ın xanaları NEZDEY_S şablonuna uyğun gəlmir,xanaların adlarını yoxla! ", "Şablonda sehv var !");
                        oConn1.Close();
                        return;
                    }
        ...
        public string my_metod_0(string s1, string s2, string s3)
        {
            ...
            cmd.CommandText = "AHALI.HS_NEZ_DEY_xlsx"; // Name_function
            ...
            cmd.Parameters.Add("v_NEW_NEZ", OracleType.VarChar);
            cmd.Parameters["v_NEW_NEZ"].Direction = ParameterDirection.Input;
            cmd.Parameters["v_NEW_NEZ"].Value = s2;

            cmd.Parameters.Add("v_OLD_NEZ", OracleType.VarChar);
            cmd.Parameters["v_OLD_NEZ"].Direction = ParameterDirection.Input;
            cmd.Parameters["v_OLD_NEZ"].Value = s3;
         ...
         private void simpleButton1_Click(object sender, EventArgs e)
        {
            ...
            File.WriteAllBytes("NEZDEY_S.xlsx", Properties.Resources.nezdey);
        }

         private void NEZDEY_S_FormClosed(object sender, FormClosedEventArgs e)
         {
             p_var.n_rez = "N";
             this.Close();
             return;
